...
async function bootstrap(): Promise<void> {
	...
	const config = app.get(AppConfigService).appPatientConfig;
	...
	new Logger("Patient App").log(
		`Patient App v${config.version} is running on: ${await app.getUrl()} with ${appConfig.env} environment`,
	);
}
bootstrap();
