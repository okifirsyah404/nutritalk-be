...
export class AppConfigLoaded {
	...
	static async appAdminConfig(): Promise<AppNutritionistConfig> {
		await ConfigModule.envVariablesLoaded;

		return {
			port: parseInt(process.env.APP_ADMIN_PORT, 10),
	...
	static async appPatientConfig(): Promise<AppNutritionistConfig> {
		await ConfigModule.envVariablesLoaded;

		return {
			port: parseInt(process.env.APP_PATIENT_PORT, 10),
